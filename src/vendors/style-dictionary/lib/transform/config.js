...
let MISSING_TRANSFORM_ERRORS = GroupMessages.GROUP.MissingRegisterTransformErrors
...
`
  ...
  if (missingTransformCount > 0) {
    let transform_warnings = GroupMessages.flush(MISSING_TRANSFORM_ERRORS).join(', ')
    let err
