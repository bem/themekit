...
let convertToBase64 = require('../utils/convertToBase64')
let UNICODE_PATTERN = /&#x([^;]+);/g
...
module.exports = {
  ...
  'color/hsl-4': {
    ...
    transformer: function(prop) {
      let color = Color(prop.value)
      let o = color.toHsl()
      let vals = `${Math.round(o.h)} ${Math.round(o.s * 100)}% ${Math.round(o.l * 100)}%`
      if (color.getAlpha() === 1) {
        return `hsl(${vals})`
      }
      return `hsl(${vals} / ${o.a})`
  ...
  'color/hex8android': {
    ...
    matcher: isColor,
    transformer: function(prop) {
      let str = Color(prop.value).toHex8()
  ...
  'color/UIColor': {
    ...
    matcher: isColor,
    transformer: function(prop) {
      let rgb = Color(prop.value).toRgb()
  ...
  'color/hex8flutter': {
    ...
    matcher: isColor,
    transformer: function(prop) {
      let str = Color(prop.value).toHex8().toUpperCase()
