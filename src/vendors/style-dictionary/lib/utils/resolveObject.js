...
let GroupMessages = require('./groupMessages')

let PROPERTY_REFERENCE_WARNINGS = GroupMessages.GROUP.PropertyReferenceWarnings
...
let updated_object; let regex; let options
  ...
  value.replace(regex, function(match, variable) {
    ...
    if (typeof ref !== 'undefined') {
      if (typeof ref === 'string') {
        ...
        if (to_ret.indexOf('{') > -1) {
          let reference = to_ret.slice(1, -1)
...
function selfRef(str, obj) {
  let i
  let ref = obj
  let array = str.split(options.separator)
  let context = current_context.join(options.separator)
