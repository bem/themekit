...
import filterProperties from '../vendors/style-dictionary/lib/filterProperties'
import GroupMessages from '../vendors/style-dictionary/lib/utils/groupMessages'
...
export function buildFile(
  ...
  let dirname = path.dirname(fullDestination)
  if (!fs.existsSync(dirname)) {
    fs.mkdirsSync(dirname)
  }

  let filteredProperties = filterProperties(dictionary, filter) as {
    allProperties: any
    properties: any
  }

  // Check for property name Collisions
  let nameCollisionObj = {} as Record<string, any>
  filteredProperties.allProperties &&
    filteredProperties.allProperties.forEach((propertyData: any) => {
  ...
  let PROPERTY_NAME_COLLISION_WARNINGS =
  ...
  console.log(
    '\n' +
      (propertyNamesCollisionCount > 0 ? '⚠️ ' : chalk.bold.green('✔︎ ')) +
      ' ' +
      fullDestination,
  )
