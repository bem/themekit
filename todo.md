...
- [ ] Изменить buildPath на что-то более подходящее, например outDir

## Legacy

Возможно в sources нужно указывать прям список платформ, в таком случае проблем не должно быть:

```json
{
  "exclude": ["./*tokens@desktop.yaml"],
  "sources": [
    "./*.tokens.yaml",
    "./*@touch.tokens.yaml",
  ]
